...
void Submarine::calculate_forces(const struct sitl_input &input, Vector3f &rot_accel, Vector3f &body_accel)
{
    ...
    // Calculate torque induced by buoytancy foams on the frame
...
void Submarine::calculate_buoyancy_torque(Vector3f &torque)
{
    ...
    const Vector3f force_position = dcm * Vector3f(0, 0, 0.15); // offset in meters
...
void Submarine::calculate_angular_drag_torque(const Vector3f &angular_velocity, const Vector3f &drag_coefficient, Vector3f &torque)
{
    ...
    // While this works, nor it is not accurate nor has the right dimensions.
    ...
    float f_d = drag_coefficient * frame_property.equivalent_sphere_area * 1000 * v_2 / 2;
    torque = angular_velocity;
    torque *= frame_property.equivalent_sphere_radius * f_d;
}
